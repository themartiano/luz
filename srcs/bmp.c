...
/*   Updated: 2021/04/26 14:13:33 by ejuliao-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	write_headers(t_scene scene, int fd)
{
	...
	file_h.type = 0x4D42;
	file_h.size = 54 + scene.x_res * scene.y_res;
	file_h.reserved = 0;
	file_h.offset = 54;
	info_h.size = 40;
	...
	info_h.planes = 1;
	info_h.bits_per_pxl = 8;
	info_h.compression = 0;
	info_h.image_size = scene.x_res * scene.y_res;
	info_h.pxls_per_meter_x = 2000;
	info_h.pxls_per_meter_y = 2000;
	info_h.color_used = 0;
	info_h.color_important = 0;
	if (write(fd, (void *)&file_h, 14) < 0 || write(fd, (void *)&info_h, 40) < 0)
		return (-1);
	return (0);
}
...
int	write_bmp(t_scene scene, t_img img, const char *file_name)
{
	...
	if (fd < 0 || write_headers(scene, fd) == -1)
		return (-1);
	while (--scene.y_res >= 0)
		if (write(fd, &img.addr[scene.y_res * img.line_length], img.line_length) < 0)
			return (-1);
	close(fd);
	free(file);
	return (0);
