...
BENCH_CASES ?= default many-objects mesh-bvh diffuse postprocess atmosphere lights emissive-geometry primitives-materials volumes obj-mesh suzanne-single suzanne-instances stormtroopers-preview stormtroopers-adaptive-tuned stormtroopers-denoise-micro
BENCH_WIDTH ?=
BENCH_HEIGHT ?=
BENCH_SAMPLES ?=
BENCH_BOUNCES ?=
...
BENCH_ADAPTIVE ?=
BENCH_DENOISE ?=
BENCH_ADAPTIVE_MIN_SAMPLES ?=
BENCH_ADAPTIVE_THRESHOLD ?=
BENCH_ADAPTIVE_CHECK_INTERVAL ?=
...
benchmark:
	...
	@docker run --rm --network none --cpus=$(BENCH_CPUS) $(BENCH_CPUSET_FLAG) --memory=$(BENCH_MEMORY) \
		...
		-e BENCH_ADAPTIVE="$(BENCH_ADAPTIVE)" \
		-e BENCH_DENOISE="$(BENCH_DENOISE)" \
		-e BENCH_ADAPTIVE_MIN_SAMPLES="$(BENCH_ADAPTIVE_MIN_SAMPLES)" \
		-e BENCH_ADAPTIVE_THRESHOLD="$(BENCH_ADAPTIVE_THRESHOLD)" \
		-e BENCH_ADAPTIVE_CHECK_INTERVAL="$(BENCH_ADAPTIVE_CHECK_INTERVAL)" \
